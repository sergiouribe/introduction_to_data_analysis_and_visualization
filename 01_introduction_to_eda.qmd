...
```{r}
model_log <- glm(species ~  bill_length_mm+ island, data = penguins, family = "binomial")
```
...
-   In this exercise, we reshape the data to a long format using **`pivot_longer()`** to create a dataset with variables for body mass, bill depth, species, and island. Then, we create a faceted scatter plot using **`ggplot2`** to explore the relationship between body mass and bill depth across different penguin species, considering the influence of island location.
    ...
    ```{r}
    penguins %>%
      select(body_mass_g, bill_depth_mm, species, island) %>%
      pivot_longer(cols = c(body_mass_g, bill_depth_mm),
                   names_to = "measurements",
                   values_to = "values") %>%
      ggplot(aes(x = values, y = values)) +
      geom_point() +
      facet_wrap(~species) +
      labs(x = "Bill Depth (mm)", y = "Values") +
      theme_bw()
    ```
...
# Reporting summaries and regression analysis
